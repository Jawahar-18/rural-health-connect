...
import { findMedicineKnowledge, MEDICINE_KNOWLEDGE_BASE } from '../../data/medicineKnowledge';
import { 
  Stethoscope, 
  Pill, 
  ArrowRightLeft, 
...
export const DoctorConsultation: React.FC = () => {
  ...
  // Prescription items state with common man guidance
  const [rxItems, setRxItems] = useState<PrescriptionItem[]>([
    { 
      ...
      durationDays: 30, 
      inStock: true,
      description: 'Blood pressure medication. Relaxes muscular blood vessels to keep circulation smooth.',
      purpose: 'Controls high blood pressure to safeguard heart and brain from strokes.',
      sideEffectsNote: 'Take with water after breakfast every morning. Do not skip doses.'
    },
    { 
      ...
      durationDays: 30, 
      inStock: true,
      description: 'Blood thinner tablet (anti-platelet). Prevents blood cells from clumping together.',
      purpose: 'Prevents blood clots inside coronary heart arteries and prevents heart attacks.',
      sideEffectsNote: 'Take strictly after night dinner to protect the stomach.'
    },
  ]);

  // New Rx form input with common man explanations
  const [newMedName, setNewMedName] = useState('');
  const [newDosage, setNewDosage] = useState('500 mg');
  const [newFreq, setNewFreq] = useState('1-0-1 (After meals)');
  const [newDays, setNewDays] = useState(5);
  const [newDescription, setNewDescription] = useState('');
  const [newPurpose, setNewPurpose] = useState('');
  const [newAdvice, setNewAdvice] = useState('');

  // Handle drug name selection or typing with auto-population of layman details
  const handleMedNameChange = (name: string) => {
    setNewMedName(name);
    if (name.trim()) {
      const info = findMedicineKnowledge(name);
      setNewDescription(info.simpleDescription);
      setNewPurpose(info.usedFor);
      setNewAdvice(info.commonAdvice);
    } else {
      setNewDescription('');
      setNewPurpose('');
      setNewAdvice('');
  ...
  const handleAddRxItem = () => {
    ...
    const info = findMedicineKnowledge(newMedName);
    setRxItems([
      ...rxItems,
      { 
        ...
        inStock: true,
        description: newDescription || info.simpleDescription,
        purpose: newPurpose || info.usedFor,
        sideEffectsNote: newAdvice || info.commonAdvice
      }
    ]);
    setNewMedName('');
    setNewDescription('');
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-soft space-y-6">
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
                  ...
                  <div key={idx} className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        ...
                        <span className="text-slate-500 ml-2">— Dosage: <strong className="text-slate-800">{item.dosage}</strong></span>
                        <span className="text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 font-bold ml-2">
                          {item.frequency}
                        </span>
                        <span className="text-slate-500 ml-2">for {item.durationDays} days</span>
                      </div>
                      <button
                    ...
                    {/* Common Man Guidance preview in Doctor's view */}
                    {(item.description || item.purpose) && (
                      <div className="p-2.5 rounded-xl bg-emerald-50/70 border border-emerald-200/80 text-[11px] space-y-1">
                        <div className="text-emerald-900 font-bold flex items-center gap-1">
                          <Sparkles className="w-3 h-3 text-emerald-600" /> Patient Drug Guide:
                        </div>
                        {item.description && (
                          <div className="text-slate-700">
                            <strong className="text-slate-900">What it is:</strong> {item.description}
                          </div>
                        )}
                        {item.purpose && (
                          <div className="text-slate-700">
                            <strong className="text-slate-900">Why used:</strong> {item.purpose}
                          </div>
                        )}
                      </div>
                    )}
              ...
              {/* Add New Drug Row */}
              <div className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-200 space-y-3.5 text-xs">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <span className="font-bold text-emerald-950 flex items-center gap-1.5">
                    <Plus className="w-4 h-4 text-emerald-700" /> Add Drug with Common-Man Explanation:
                  </span>
                  <span className="text-[11px] text-emerald-700 font-medium">
                    Auto-fills simple, patient-friendly guidance
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {MEDICINE_KNOWLEDGE_BASE.slice(0, 7).map((med) => (
                    <button
                      key={med.genericName}
                      type="button"
                      onClick={() => handleMedNameChange(`Tab. ${med.genericName}`)}
                      className="px-2 py-0.5 rounded-lg bg-white border border-emerald-200 text-[11px] font-semibold text-emerald-900 hover:bg-emerald-100 hover:border-emerald-300 transition-colors cursor-pointer"
                    >
                      {med.genericName}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-5 gap-2.5">
                  <input
                    ...
                    className="sm:col-span-2 border border-slate-300 rounded-xl p-2.5 font-semibold text-slate-900 bg-white"
                  />
                  <input
                    type="text"
                    ...
                    className="border border-slate-300 rounded-xl p-2.5 font-medium bg-white"
                  />
                  <input
                    type="text"
                    placeholder="Timing (1-0-1 After meals)"
                    value={newFreq}
                ...
                {/* Patient Plain-Language Fields */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                  <div>
                    <label className="block text-[11px] font-bold text-emerald-950 mb-1">
                      What is this drug? (Plain language definition for common man):
                    </label>
                    <input
                      type="text"
                      placeholder="e.g. Fever-reducing and pain-relieving medicine"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-emerald-950 mb-1">
                      Why is it used for? (Clinical purpose for patient):
                    </label>
                    <input
                      type="text"
                      placeholder="e.g. Relieves headache, body ache, and fever"
                      value={newPurpose}
                ...
                <div className="flex justify-end pt-1">
                  <button
                    ...
                    className="px-5 py-2 bg-gov-green-700 hover:bg-gov-green-800 text-white rounded-xl font-bold text-xs flex items-center gap-1.5 shadow-sm transition-transform active:scale-95 cursor-pointer"
                  >
                    <Plus className="w-4 h-4" /> Add Drug to Prescription
                  </button>
                </div>
              </div>
            </div>
