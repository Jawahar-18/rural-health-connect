...
export const DoctorPrescriptionsView: React.FC = () => {
  ...
  const handlePrint = (rxId?: string) => {
    if (rxId) {
      console.log('Printing prescription:', rxId);
    }
    window.print();
  };
        ...
        <div className="space-y-6">
          {filteredPrescriptions.map((rx) => (
            ...
            >
              ...
              {rx.followUpDate && (
                <div className="flex items-center justify-between p-3 rounded-xl bg-indigo-50/70 border border-indigo-200 text-xs">
                  <div className="flex items-center gap-2 text-indigo-900 font-bold">
                    <Calendar className="w-4 h-4 text-indigo-600" />
                    <span>Scheduled Review / Follow-up Visit:</span>
                    <span className="font-black underline decoration-indigo-400">{rx.followUpDate}</span>
                  </div>
                  <span className="text-[11px] text-indigo-700 font-semibold bg-white px-2 py-0.5 rounded-md border border-indigo-200">
                    ASHAs & ANMs Notified
                  </span>
                </div>
              )}
